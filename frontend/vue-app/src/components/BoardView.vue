...
<div v-if="selectedPost" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-sm animate-fade-in">
  <div class="bg-white/95 backdrop-blur-2xl w-full max-w-xl rounded-[2rem] p-6 sm:p-8 shadow-2xl border border-white space-y-5 max-h-[90vh] overflow-y-auto">
    <div class="flex justify-between items-start">
      ...
      <button @click="closeDetail" class="text-slate-400 hover:text-slate-600 font-bold w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 transition text-lg">✕</button>
    </div>
    ...
    <div class="flex justify-between items-center text-xs pt-2">
      <div class="flex items-center gap-2">
        ...
        <button v-if="!pwdMode" @click="pwdMode = 'edit'" class="px-4 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold transition">수정</button>
        <button v-if="!pwdMode" @click="pwdMode = 'delete'" class="px-4 py-1.5 rounded-xl bg-slate-100 hover:bg-rose-50 hover:text-rose-600 text-slate-600 font-bold transition">삭제</button>
      </div>
    ...
    <!-- 비밀번호 확인 영역 -->
    <div v-if="pwdMode" class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3 mt-4 animate-fade-in shadow-inner">
    ...
    <div class="pt-4 border-t border-slate-100 space-y-3">
      <p class="text-xs font-bold text-slate-700 flex items-center">
        <span class="mr-1">💬</span> 댓글 {{ comments.length }}개
      </p>

      <div class="space-y-2 max-h-56 overflow-y-auto pr-1">
        <div v-for="comment in comments" :key="comment.id" class="flex items-start justify-between gap-2 bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 shadow-sm">
          <div class="min-w-0">
            <p class="text-sm text-slate-700 whitespace-pre-wrap break-words">{{ comment.content }}</p>
            <p class="text-[11px] text-slate-400 mt-1">{{ formatDate(comment.created_at) }}</p>
          </div>
          <button @click="deleteComment(comment)" class="shrink-0 text-slate-300 hover:text-rose-500 transition text-xs font-bold px-2 py-1">✕</button>
        </div>
        <p v-if="comments.length === 0" class="text-xs text-slate-400 text-center py-4">아직 댓글이 없어요. 첫 댓글을 남겨보세요!</p>
      </div>

      <div class="bg-slate-50 p-3 rounded-2xl border border-slate-200 space-y-2 shadow-inner">
        <textarea v-model="newComment.content" rows="2" placeholder="댓글을 남겨주세요!" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-sky-400/50 outline-none transition-all resize-none"></textarea>
        <div class="flex gap-2">
          <input v-model="newComment.password" type="password" placeholder="댓글 비밀번호" class="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-400/20">
          <button @click="submitComment" class="px-5 py-2 bg-sky-500 hover:bg-sky-600 text-white rounded-xl text-xs font-bold shadow transition shrink-0">등록</button>
        </div>
      </div>
    </div>
  </div>
